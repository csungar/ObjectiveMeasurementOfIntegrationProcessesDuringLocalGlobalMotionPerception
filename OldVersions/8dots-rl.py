...
radius = 10 # the radius between a pair of circles
...
flag = True
while flag==True:
    textStim = visual.TextStim(win, text = "You will see a bunch of dots rotate.... Please press space bar to start.")
    textStim.draw()
    win.flip()
...
while flag==True:
    ...
    globForm.opacities = 0.2*np.sin(10.5*theta)+0.75
    ...
    if right==True:
        textStim = visual.TextStim(win, text = "right")
    elif left==True:
        textStim = visual.TextStim(win, text = "left")
    textStim.draw()
    win.flip()
